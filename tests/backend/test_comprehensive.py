...
def test_backend_endpoints():
    ...
    # Test 1: Quran Search
    print("\n[TEST 1] Testing /api/quran/search...")
    try:
        response = requests.get(f"{BASE_URL}/api/quran/search", params={"keyword": "patience"}, timeout=10)
        print(f"  Status: {response.status_code}")
        if response.status_code == 200:
            data = response.json()
            print(f"  ✓ SUCCESS: Found {len(data)} verses")
            if data:
                print(f"  Sample: {data[0].get('surah', 'N/A')} - {data[0].get('text', 'N/A')[:50]}...")
        else:
    ...
    print("\n[TEST 2] Testing /api/hadith/search...")
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data:
                print(f"  ✓ SUCCESS: Found hadith")
                print(f"  Sample: {str(data.get('text', 'N/A'))[:50]}...")
            else:
                print(f"  ✓ SUCCESS: No hadith found (expected for some searches)")
        else:
            print(f"  ✗ FAILED: {response.text}")
    except Exception as e:
        print(f"  ✗ ERROR: {e}")
    ...
    # Test 3: Get API Key
    print("\n[TEST 3] Testing /api/get-api-key...")
    try:
        ...
        if response.status_code == 200:
            ...
            if api_key:
                print(f"  ✓ SUCCESS: API key loaded (length: {len(api_key)})")
            else:
                print(f"  ⚠ WARNING: No API key in .env")
        else:
            print(f"  ✗ FAILED: {response.text}")
    except Exception as e:
        print(f"  ✗ ERROR: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('error'):
                ...
                print(f"  ✓ SUCCESS: Received guidance")
...
def test_static_files():
    ...
    for name, path in files_to_test:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"  ✓ SUCCESS: {name} loaded ({len(response.content)} bytes)")
            else:
